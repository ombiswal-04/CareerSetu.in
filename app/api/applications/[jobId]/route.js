...
export async function POST(request, { params }) {
    try {
        ...
        const existingApp = await Application.findOne({ user: user._id, job: jobId });
        if (existingApp) return Response.json({ message: 'Already applied' }, { status: 400 });

        const application = await Application.create({ user: user._id, job: jobId });
